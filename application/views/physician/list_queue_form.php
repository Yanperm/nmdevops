    ...
    <div class="col-xl-6 col-lg-6 col-md-6">
        ...
        ?>
                ...
                <div class="alert alert-info text-center" role="alert">
                    <p> ท่านได้ทำการนัดหมอในวันที่
                        <?php echo $date; ?>
                    </p>
                    <p>คิวที่จอง :
                        <?php echo $queueBooked?>
                    </p>
                    <p>เรียบร้อยแล้ว</p>
                </div>
                <?php endif; ?>
    ...
    <div class="col-md-6">
        <form action="<?php echo base_url('physician/insertQueue'); ?>" method="post" id="add-queue-form">
            ...
            <div id="form-queue" style='display:none'>
                ...
                <div class="form-group">
                    <label>นามสกุล</label>
                    <input type="text" class="form-control" placeholder="" id="lname" name="lastname_booking" required>
                </div>
                <div class="form-group">
                    <label>เบอร์โทรศัพท์ติดต่อ</label>
                    <input type="text" class="form-control" placeholder="" id="telephone" name="telephone" required>
                </div>
                <div class="form-group">
                    <label>Line Id</label>
                    <input type="text" class="form-control" placeholder="" id="line_id" name="line_id">
                </div>
                ...
                <div class="form-group">
                    ...
                    <button class="btn btn-default" type="button" onclick="resetData()">ล้างข้อมูล</button>
                </div>
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (Object.keys(data.member).length === 0) {
                    ...
                    Swal.fire({
                        title: 'เมลล์นี้มีการใช้งาน!',
                        text: "บัญชีคุณ " + data.member.CUSTOMERNAME + " ใช่หรือไม่",
                        icon: 'warning',
                        ...
                        if (result.isConfirmed) {
                            let name = data.member.CUSTOMERNAME.split(" ");

                            $("#fname").val(name[0]);
                            $("#lname").val(name[1]);
                            $("#telephone").val(data.member.PHONE);
                            $("#line_id").val(data.member.LINEID);
